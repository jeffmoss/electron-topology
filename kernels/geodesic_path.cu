// Phase 1: Geodesic path-length scan — ALL-PAIRS version
//
// Each thread evaluates one (rho, pair_idx) where pair_idx indexes into
// a precomputed table of (p1,q1,p2,q2) winding number pairs.
//
// For each pair, we compute:
//   L1 = path_length(p1, q1, rho)   [numerator mode]
//   L2 = path_length(p2, q2, rho)   [denominator mode]
//   ratio = L1 / L2
//   score = |ratio - TARGET_RATIO|
//
// This tests ALL pairs of winding numbers, not just against a fixed
// ground state. The GPU explores which pair of modes on the same torus
// produces a mass ratio matching the muon/electron ratio.
//
// Block-level reduction selects the single best candidate per block.

#include "common.cuh"

extern "C" __global__ void geodesic_scan(
    int          num_rho,       // number of rho grid points
    double       rho_min,       // smallest rho value
    double       rho_step,      // spacing between rho values
    const int   *p1_vals,       // numerator poloidal winding
    const int   *q1_vals,       // numerator toroidal winding
    const int   *p2_vals,       // denominator poloidal winding
    const int   *q2_vals,       // denominator toroidal winding
    int          num_pairs,     // number of (p1,q1,p2,q2) pairs
    CandidateResult *block_results  // output: one per block
) {
    unsigned int tid = threadIdx.x;
    unsigned int gid = blockIdx.x * blockDim.x + tid;
    unsigned int total_work = (unsigned int)num_rho * (unsigned int)num_pairs;

    // Default: worst possible score so non-participating threads lose reduction
    double score       = 1.0e30;
    double rho         = 0.0;
    int    my_p        = 0;
    int    my_q        = 0;
    double path_length = 0.0;
    double ratio       = 0.0;

    if (gid < total_work) {
        // Decode linear index into (rho_idx, pair_idx)
        int rho_idx  = gid / num_pairs;
        int pair_idx = gid % num_pairs;

        rho = rho_min + (double)rho_idx * rho_step;

        int p1 = p1_vals[pair_idx];
        int q1 = q1_vals[pair_idx];
        int p2 = p2_vals[pair_idx];
        int q2 = q2_vals[pair_idx];

        // Compute both path lengths via GL quadrature
        double L1 = compute_path_length(p1, q1, rho);
        double L2 = compute_path_length(p2, q2, rho);

        path_length = L1;  // store numerator path for diagnostics

        // Ratio: L1 / L2 (longer path mode / shorter path mode)
        if (L2 > 1.0e-15) {
            ratio = L1 / L2;
        }

        score = fabs(ratio - TARGET_RATIO);

        // Encode pair info: p = p1*1000 + q1, q = p2*1000 + q2
        my_p = p1 * 1000 + q1;
        my_q = p2 * 1000 + q2;
    }

    // Block-level reduction to find the best candidate in this block
    block_reduce_min_candidate(score, rho, my_p, my_q, path_length, ratio,
                               block_results);
}
